...
- ⚠️ **Зависание при сканировании шины I2C через FTDI (FT232H)**
  - **Описание:** Полный перебор адресов в диапазоне `0x08-0x78` при использовании адаптеров FTDI (через библиотеку `pyftdi`) вызывает аппаратное или программное зависание потока выполнения. 
  - **Временное решение:** В модуле `core/bus_scanner.py` для FTDI-шин временно жестко ограничено сканирование только по известным адресам устройств PMBus (LTM4673 и др.): `[0x5B, 0x5C, 0x58, 0x5A, 0x5D, 0x4C, 0x4D]`. Для системных шин Linux и адаптеров CH341 по-прежнему доступно полное сканирование.
  - *Статус:* В коде `core/pmbus_device.py` уже внедрены таймауты (`with_timeout`) и пропуск опасных блочных команд (`FTDI_SKIP_BLOCK`), однако логика самого метода `scan_bus` требует дальнейшей отладки механизма обработки исключений и таймаутов на уровне портов `pyftdi`. Исправление планируется в следующих релизах.
